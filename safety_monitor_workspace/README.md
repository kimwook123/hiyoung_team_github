...
- 좌측 카메라 리스트와 중앙 Live Monitoring 그리드로 여러 클라이언트를 모니터링합니다.`r`n- 각 클라이언트의 이벤트 목록, 썸네일, 클립 정보를 서버에 요청해 조회합니다.`r`n- 이벤트 클릭 시 해당 카메라 타일에서 클립을 재생합니다.`r`n- 위험구역 이벤트 클립에는 이벤트 당시 ROI 사각형을 오버레이합니다.`r`n- 각 클라이언트별 위험상황 룰 적용 설정을 서버로 전송합니다.`r`n- 테스트용 DB Clear 버튼으로 서버 이벤트 DB/클립/썸네일을 초기화할 수 있습니다.
