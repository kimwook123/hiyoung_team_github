...
다른 PC 서버를 쓸 때는 GUI 상단 `서버 주소` 입력이나 `run_gui.bat` 초기 입력으로 API base URL을 맞춥니다.
...
- 이벤트 상세의 `클립 재생`은 별도 새 창이 아니라 현재 화면의 대상 패널에서 재생 모드로 전환
- 클립 재생 중 `클립 닫기`로 원래 영상/스트림으로 복귀 가능
- 비활성 영상은 자동 pause 하지 않음
- 여러 타일이 동시에 재생될 수 있으며 오디오는 활성 타일만 출력
- 서버 등록 소스 목록에서 현재 상태와 진행도를 확인 가능
- 서버 등록 소스는 `화면에 열기`로 다시 복원 가능
- 서버 등록 소스의 휴지통 버튼은 서버 완전 삭제와 동일하게 동작
