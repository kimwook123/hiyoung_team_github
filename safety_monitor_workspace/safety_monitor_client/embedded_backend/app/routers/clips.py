...
from app.config import CLIENT_CLIP_DIR
...
def list_clips() -> ClipListResponse:
    ...
    items = [
        ...
        for clip_path in sorted(CLIENT_CLIP_DIR.glob("*.mp4"))
...
def get_clip(clip_name: str) -> FileResponse:
    ...
    clip_path = (CLIENT_CLIP_DIR / normalized_name).resolve()
    try:
        clip_path.relative_to(CLIENT_CLIP_DIR)
...
def _build_unique_clip_path(file_name: str) -> Path:
    candidate = (CLIENT_CLIP_DIR / file_name).resolve()
    stem = Path(file_name).stem
    suffix = Path(file_name).suffix
    index = 1

    while candidate.exists():
        candidate = (CLIENT_CLIP_DIR / f"{stem}_{index}{suffix}").resolve()
        index += 1

    try:
        candidate.relative_to(CLIENT_CLIP_DIR)
    except ValueError as error:
        raise HTTPException(status_code=400, detail="invalid clip path") from error
