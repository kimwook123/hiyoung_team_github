...
def create_event(
    ...
    # 이벤트 생성은 서버 내부의 server_event_processor가 frame_detections를 처리하면서만 수행합니다.
    # 클라이언트가 룰을 판단해 /api/events로 올리는 예전 흐름은 허용하지 않습니다.
    raise HTTPException(
        status_code=410,
        detail="event creation is handled by the server event processor from frame detections",
    )
